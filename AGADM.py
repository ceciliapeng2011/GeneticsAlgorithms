...
class DiversityMutationAGA:
    ...
    def crossover(self, parent_list):
        ...
        for i in range(0, len(parent_list), 2): #FIXME：How to choose parents?
    ...
    def mutation_adptive(self, new_population):
        '''
        This function is to do mutation on the input new chromosome list
        :param new_population: the list of new chromosome
        :return: new chromosome list after mutation
...
if __name__ == "__main__":
    ...
    '''
    Initialize AGADM algorithm context
    '''
    AGADM = DiversityMutationAGA(knapsack_fitness_function, params, "tournament", 0.05, 1, 0.95, 2, True, params.get('tournament_size'))

    '''
    Generate binary population with given binary chromosume length
    '''
    AGADM.generate_binary_population(params.get('pop_size'), item_num)

    '''
    Do Evaluation and sorting on current population
    '''
    AGADM.evaluate_population()
    AGADM.sort_population()
     
    '''
    Selection
    '''
    parent_list = AGADM.select_parents()
    ...
    for index in range(iter_number):
        ...
        '''
        perform crossover with adaptive probability;
        '''
        next_population = AGADM.crossover(parent_list)
        
        '''
        perform diversity-guided mutation
        A1 = 0 . 01, Az -----0. 25, ks = 0 . 60, k6 = 0.03, and kz --~ 0. 000 01
        '''
        AGADM.mutation_diversity(0.01, 0.25, 0.60, 0.03, 0.00001, params.get('item_values'))
        next_population = AGADM.mutation(next_population)
        
        '''
        perform mutation with adaptive probability;
        '''
        next_population = AGADM.mutation_adptive(next_population)

        '''
        Mating and update current population
        '''
        AGADM.mating(next_population)
